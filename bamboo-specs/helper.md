...
2. setup email server for bamboo to send notifications whenever the github repo get updated
    -> bamboo administration => communication => mail server
3. enable 'Allow anonymous users to trigger remote repository change detection and Bamboo Specs detection' in 
...
4. now go to specs menu in the top
...
5. now go to projects => 'your project' => project settings (will be availble in right corner) => Bamboo specs repositories => add repo



For Docker Agent type,
1. make sure eks/scripts available in whichever the directory you mount to docker container
2. make sure docker container has internet access
3. make sure you use the Project key from docker specs yaml
